  ...
  return (
    <>
      ...
      {/* Fullscreen "new page" push on mobile (no backdrop, slides in from
          the right); centered modal card on desktop (backdrop + slide-from-
          bottom, unchanged) — same treatment as VarietySection's sheet. */}
      {selected && (
        <Portal>
          <div
          ...
          >
            <div
              className="bg-card w-full h-full md:h-auto md:w-auto md:max-w-lg md:rounded-2xl md:shadow-2xl md:max-h-[85vh] overflow-hidden flex flex-col animate-in slide-in-from-right md:slide-in-from-right-0 md:slide-in-from-bottom md:fade-in md:zoom-in-95 duration-300"
            ...
            >
              <div className="relative h-48 md:h-60 shrink-0 bg-muted overflow-hidden">
                ...
                {/* One labelled control at every size — see VarietySection. */}
                <button
                  onClick={() => setSelected(null)}
